...
contract Asm {
    function asmLoops() public pure returns (uint256 _r) {
        uint256 i = 0;
        assembly {
            for { i := 0 } lt(i, 10) { i := add(i, 1) } { _r := add(_r, 1) }
            mstore(0x0, _r)
            return(0x0, 32)
        }
    }

    function asmSumOfArray(uint256[] memory arr) public pure returns (uint256 sum) {
        assembly {
